...
import { APIPromise } from 'superwall-api/core/api-promise';

import util from 'node:util';
import SuperwallAPI from 'superwall-api';
import { APIUserAbortError } from 'superwall-api';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['SUPERWALL_API_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: SuperwallAPI) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['SUPERWALL_API_LOG'] = 'debug';
    const client = new SuperwallAPI({
  logger: logger,
  apiKey: 'My API Key',
  bearerToken: 'My Bearer Token',
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['SUPERWALL_API_LOG'] = 'not a log level';
    const client = new SuperwallAPI({
  logger: logger,
  apiKey: 'My API Key',
  bearerToken: 'My Bearer Token',
    ...
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'SUPERWALL_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  bearerToken: 'My Bearer Token',
    ...
    afterEach(() => {
      process.env['SUPERWALL_API_BASE_URL'] = undefined;
    });
  ...
  bearerToken: 'My Bearer Token',
    ...
    test('in request options overridden by env variable', () => {
      process.env['SUPERWALL_API_BASE_URL'] = 'http://localhost:5000/env';
      const client = new SuperwallAPI({ apiKey: 'My API Key', bearerToken: 'My Bearer Token' });
  ...
  bearerToken: 'My Bearer Token',
    ...
    // default
    const client2 = new SuperwallAPI({ apiKey: 'My API Key', bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['SUPERWALL_API_API_KEY'] = 'My API Key';
    process.env['SUPERWALL_API_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new SuperwallAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['SUPERWALL_API_API_KEY'] = 'another My API Key';
    process.env['SUPERWALL_API_BEARER_TOKEN'] = 'another My Bearer Token';
    const client = new SuperwallAPI({ apiKey: 'My API Key', bearerToken: 'My Bearer Token' });
    expect(client.apiKey).toBe('My API Key');
    expect(client.bearerToken).toBe('My Bearer Token');
  });
