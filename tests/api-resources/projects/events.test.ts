...
describe('resource events', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.projects.events.list('project_id', {
    after: 'after',
    before: 'before',
    channel: 'channel',
    event_types: ['string'],
  ...
  test.skip('retryDelivery: only required params', async () => {
    const responsePromise = client.projects.events.retryDelivery('event_id', { project_id: 'project_id', endpoint_id: 'endpoint_id' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('retryDelivery: required and optional params', async () => {
    const response = await client.projects.events.retryDelivery('event_id', { project_id: 'project_id', endpoint_id: 'endpoint_id' });
  });
});
