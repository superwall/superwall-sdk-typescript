...
export interface ClientOptions {
  /**
   * Defaults to process.env['SUPERWALL_API_API_KEY'].
   */
  apiKey?: string | undefined;

  /**
   * Defaults to process.env['SUPERWALL_API_BEARER_TOKEN'].
   */
   ...
   *
   * Defaults to process.env['SUPERWALL_API_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SUPERWALL_API_LOG'] or 'warn' if it isn't set.
   */
...
export class SuperwallAPI {
   ...
   */
  constructor({
    baseURL = readEnv('SUPERWALL_API_BASE_URL'),
    apiKey = readEnv('SUPERWALL_API_API_KEY'),
    bearerToken = readEnv('SUPERWALL_API_BEARER_TOKEN'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    const options: ClientOptions = {
      ...
      bearerToken,
      ...opts,
      baseURL: baseURL || `https://api.superwall.com`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? SuperwallAPI.DEFAULT_TIMEOUT /* 1 minute */;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.superwall.com';
  }
  ...
  protected async oauth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
