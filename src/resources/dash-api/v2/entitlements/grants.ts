...
export class Grants extends APIResource {
  ...
  create(id: string, body: GrantCreateParams, options?: RequestOptions): APIPromise<GrantCreateResponse> {
    return this._client.post(path`/dash-api/v2/entitlements/${id}/grants`, { body, ...options });
  }
  ...
  list(
    ...
    return this._client.get(path`/dash-api/v2/entitlements/${id}/grants`, { query, ...options });
  }
  ...
  delete(
    ...
    const { id } = params;
    return this._client.delete(path`/dash-api/v2/entitlements/${id}/grants/${grantID}`, options);
  }
}
